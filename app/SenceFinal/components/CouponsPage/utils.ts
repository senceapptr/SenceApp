...
export const getStatusColor = (status: 'live' | 'won' | 'lost'): [string, string] => {
  switch (status) {
    case 'live': return ['#C4B5FD', '#A78BFA'];
    case 'won': return ['#A7F3D0', '#6EE7B7'];
    case 'lost': return ['#FCA5A5', '#F87171'];
...
export const getStatusBadge = (status: 'live' | 'won' | 'lost') => {
  switch (status) {
    case 'live': return { text: '⏰ 7 gün 0 saat', color: '#8B5CF6' };
...
export const getModalGradientColors = (status: 'live' | 'won' | 'lost'): [string, string] => {
  switch (status) {
    case 'live': return ['#8B5CF6', '#A855F7'];
