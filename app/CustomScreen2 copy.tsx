...
export default function CustomScreen2() {
  // useSafeAreaInsets ve StatusBar kaldırıldı
  ...
  // slideButton için scale animasyonu (6px küçülme için scale oranı hesaplanacak)
  const slideButtonScale = useRef(new Animated.Value(1)).current;

  // Devam yazısı için opacity animasyonu
  const devamTextOpacity = useRef(new Animated.Value(0)).current;
  // Devam yazısı için fontSize animasyonu
  const devamTextFontSize = useRef(new Animated.Value(28)).current;

  // Oklar için opacity animasyonu
  const arrowsOpacity = useRef(new Animated.Value(1)).current;

  // Devam butonu için scale animasyonu
  const devamButtonScale = useRef(new Animated.Value(1)).current;
  // Devam butonunun görünürlüğü
  const [showDevamButton, setShowDevamButton] = useState(false);
  // Devam butonu scale değerini takip eden ref
  const devamButtonScaleValue = useRef(1);
  React.useEffect(() => {
    const id = devamButtonScale.addListener(({ value }) => {
      devamButtonScaleValue.current = value;
    });
    return () => devamButtonScale.removeListener(id);
  }, [devamButtonScale]);
  ...
  React.useEffect(() => {
    if (completed) {
      ...
      // "tek doğru cevap." yazısından hemen sonra küçülme animasyonu başlasın
      setTimeout(() => {
        // slideButton'un scale'ini küçült (6px her kenardan)
        const scaleX = (0.95 * SCREEN_WIDTH - 12) / (0.95 * SCREEN_WIDTH);
        const scaleY = (96 - 12) / 96;
        const minScale = Math.min(scaleX, scaleY);
        Animated.timing(slideButtonScale, {
          toValue: minScale,
          duration: 350,
          easing: Easing.out(Easing.cubic),
          useNativeDriver: true,
        }).start(() => {
          // slideButton küçülme animasyonu tamamlandığında Devam butonunu göster
          setShowDevamButton(true);
          devamButtonScale.setValue(minScale); // Devam butonu scale'ı slideButton'ın küçülmüş haliyle başlasın
          devamButtonScaleValue.current = minScale;
        });
        // Devam yazısı fade-in ve fontSize animasyonu
        devamTextOpacity.setValue(0);
        devamTextFontSize.setValue(28);
        Animated.parallel([
          Animated.timing(devamTextOpacity, {
            toValue: 1,
            duration: 350,
            easing: Easing.out(Easing.cubic),
            useNativeDriver: true,
          }),
          Animated.timing(devamTextFontSize, {
            toValue: 24,
            duration: 350,
            easing: Easing.out(Easing.cubic),
            useNativeDriver: false, // fontSize için native driver desteklenmez
          }),
        ]).start();
      }, 2000); // 2.00 saniye sonra başlasın
      // "tek doğru cevap." yazısından 0.75s sonra küçülme animasyonu başlasın
      // setTimeout(() => {
      //   // slideButton'un scale'ini küçült (6px her kenardan)
      //   // slideButton genişliği: %95, yüksekliği: 96
      //   // 6px sağ-sol ve üst-alt küçülme için scaleX = (width-12)/width, scaleY = (height-12)/height
      //   const scaleX = (0.95 * SCREEN_WIDTH - 12) / (0.95 * SCREEN_WIDTH);
      //   const scaleY = (96 - 12) / 96;
      //   Animated.timing(slideButtonScale, {
      ...
      //     useNativeDriver: true,
      //   }).start();
      // }, 2250 + 750); // "tek doğru cevap." yazısı fade-in + 0.75s
    }
  }, [completed]);
              ...
              <View style={styles.bottomArea}>
                ...
                {!showDevamButton && (
                  <Animated.View
                    style={[
                      styles.slideButton,
                      { transform: [{ scale: slideButtonScale }] },
                    ]}
                    onLayout={e => {
                      setButtonWidth(e.nativeEvent.layout.width);
                      parentLeft.current = e.nativeEvent.layout.x;
                  ...
                  >
                    ...
                    {/* Devam yazısı: buton küçülmeye başladığında ekrana fade-in ile gelsin */}
                    <Animated.View
                      pointerEvents="none"
                      style={{
                        position: 'absolute',
                        ...
                        alignItems: 'center',
                        opacity: devamTextOpacity,
                        zIndex: 5,
                      }}
                    >
                      <Animated.Text style={{ color: '#fff', fontWeight: 'bold', fontSize: devamTextFontSize, textAlign: 'center' }}>Devam</Animated.Text>
                    </Animated.View>
                  </Animated.View>
                )}
                {/* Devam butonu birleşik ve animasyonlu */}
                {showDevamButton && (
                  <Animated.View
                    style={[
                      styles.slideButton,
                      { transform: [{ scale: devamButtonScale }] },
                  ...
                  >
                    <TouchableOpacity
                      ...
                      activeOpacity={0.8}
                      onPress={() => {
                        // Önce hızlıca küçült (scale 1 -> 0.5)
                        const currentScale = devamButtonScaleValue.current;
                        Animated.sequence([
                          Animated.timing(devamButtonScale, {
                            toValue: currentScale * 0.5,
                            duration: 150,
                          ...
                          // Büyüme animasyonu tamamlandığında animasyonlu geçiş ekranına yönlendir
                          router.replace('/SenceHome/SenceTransitionScreen');
                        });
                      }}
                    >
                      <Animated.Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 24, textAlign: 'center', opacity: devamTextOpacity }}>Devam</Animated.Text>
                    </TouchableOpacity>
                  </Animated.View>
                )}
                {/* Login info */}
