...
export function SlideOutMenu({ isOpen, onClose, onNavigate, children }: SlideOutMenuProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Close animation - keep overlay visible longer
      Animated.parallel([
        Animated.timing(slideAnim, {
          toValue: 0,
          duration: 300,
        ...
        Animated.timing(overlayOpacity, {
          toValue: 0,
          duration: 350, // Longer duration to keep purple visible
          useNativeDriver: true,
        }),
      ]).start(() => {
        setIsAnimating(false);
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Dark Background Layer for Scaled Content */}
      {(isOpen || isAnimating) && (
        <Animated.View
          style={[
            styles.scaledContentBackground,
            {
      ...
      {/* Main Content (scaled and moved to left) */}
      <Animated.View
        style={[
          styles.mainContent,
          {
