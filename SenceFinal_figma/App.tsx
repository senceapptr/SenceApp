...
import { AlternativeLeaguePage } from './components/AlternativeLeaguePage';
...
export default function App() {
  ...
  const [showAlternativeCoupons, setShowAlternativeCoupons] = useState(false);
  const [showAlternativeLeague, setShowAlternativeLeague] = useState(false);
  const [alternativeProfileDropdown, setAlternativeProfileDropdown] = useState(false);
  ...
  // Check if we should hide navigation (only for trivia mode OR alternative pages)
  const shouldHideNavigation = appState.currentPage === 'trivia' || showAlternativeHome || showAlternativeSearch || showAlternativeCoupons || showAlternativeLeague;
  ...
  const handleAlternativeSearchClick = () => {
    setShowAlternativeSearch(true);
  };

  const handleAlternativeCouponsClick = () => {
    setShowAlternativeCoupons(true);
  };

  const handleAlternativeLeagueClick = () => {
    setShowAlternativeLeague(true);
  };

  const handleNavigateToDiscover = () => {
    setShowAlternativeCoupons(false);
    setShowAlternativeSearch(true);
  };

  // Alternative pages profile dropdown handlers
  const handleAlternativeProfileClick = () => {
    setAlternativeProfileDropdown(!alternativeProfileDropdown);
  };

  const handleCloseAlternativeDropdown = () => {
    setAlternativeProfileDropdown(false);
  };

  // Reset alternative dropdown when leaving alternative pages
  const resetAlternativeState = () => {
    setAlternativeProfileDropdown(false);
    setShowAlternativeHome(false);
    setShowAlternativeSearch(false);
    setShowAlternativeCoupons(false);
    setShowAlternativeLeague(false);
  };

  const handleAlternativeProfilePageClick = () => {
    resetAlternativeState();
    // Navigate to profile page
    appState.setCurrentPage('profile');
  };

  const handleAlternativeNotificationsClick = () => {
    setAlternativeProfileDropdown(false);
    handleNotificationsClick();
  };

  const handleAlternativeTriviaClick = () => {
    resetAlternativeState();
    // Navigate to trivia page
    appState.setCurrentPage('trivia');
  };

  const handleAlternativeSwipeClick = () => {
    resetAlternativeState();
    // Navigate to predict/swipe page
    appState.setCurrentPage('predict');
  };

  const handleAlternativeTasksClick = () => {
    resetAlternativeState();
    // Show tasks page
    setShowTasks(true);
  };

  const handleAlternativeMarketClick = () => {
    resetAlternativeState();
    // Navigate to market page
    appState.setCurrentPage('market');
  };

  const handleAlternativeWriteQuestionClick = () => {
    resetAlternativeState();
    // Show write question page
    setShowWriteQuestion(true);
  };

  const handleAlternativeEditProfileClick = () => {
    resetAlternativeState();
    // Navigate to edit profile page
    appState.setCurrentPage('edit-profile');
  };

  const handleAlternativeSettingsClick = () => {
    resetAlternativeState();
  ...
  // Show Alternative Search Page
  if (showAlternativeSearch) {
    return (
      <div className="min-h-screen bg-[#F2F3F5]">
        <div className="w-full min-h-screen relative bg-[#F2F3F5]">
          <AlternativeSearchPage
            onQuestionDetail={handleQuestionDetail}
            onVote={handleVote}
            showProfileDropdown={alternativeProfileDropdown}
            onProfileClick={handleAlternativeProfileClick}
            onCloseDropdown={handleCloseAlternativeDropdown}
            onProfilePageClick={handleAlternativeProfilePageClick}
            onNotificationsClick={handleAlternativeNotificationsClick}
            onTriviaClick={handleAlternativeTriviaClick}
            onSwipeClick={handleAlternativeSwipeClick}
            onTasksClick={handleAlternativeTasksClick}
            onMarketClick={handleAlternativeMarketClick}
            onWriteQuestionClick={handleAlternativeWriteQuestionClick}
            onEditProfileClick={handleAlternativeEditProfileClick}
            onSettingsClick={handleAlternativeSettingsClick}
          />
          
          <BottomTabs currentPage="search" onPageChange={(page) => {
            if (page !== "search") {
              setShowAlternativeSearch(false);
              appState.setCurrentPage(page);
            }
  ...
  // Show Alternative Coupons Page
  if (showAlternativeCoupons) {
    return (
      <div className="min-h-screen bg-[#F2F3F5]">
        <div className="w-full min-h-screen relative bg-[#F2F3F5]">
          <MyBetsPage
            onQuestionDetail={handleQuestionDetail}
            onVote={handleVote}
            onNavigateToAlternativeCoupons={handleNavigateToDiscover}
            showProfileDropdown={alternativeProfileDropdown}
            onProfileClick={handleAlternativeProfileClick}
            onCloseDropdown={handleCloseAlternativeDropdown}
            onProfilePageClick={handleAlternativeProfilePageClick}
          ...
          <BottomTabs currentPage="predict" onPageChange={(page) => {
            if (page !== "predict") {
              setShowAlternativeCoupons(false);
              appState.setCurrentPage(page);
            }
  ...
  // Show Alternative League Page
  if (showAlternativeLeague) {
    return (
      <div className="min-h-screen bg-[#F2F3F5]">
        <div className="w-full min-h-screen relative bg-[#F2F3F5]">
          <AlternativeLeaguePage
            onBack={() => setShowAlternativeLeague(false)}
            showProfileDropdown={alternativeProfileDropdown}
            onProfileClick={handleAlternativeProfileClick}
            onCloseDropdown={handleCloseAlternativeDropdown}
            onProfilePageClick={handleAlternativeProfilePageClick}
          ...
          <BottomTabs currentPage="league" onPageChange={(page) => {
            if (page !== "league") {
              setShowAlternativeLeague(false);
              appState.setCurrentPage(page);
            }
    ...
    <div className="min-h-screen bg-[#F2F3F5]">
      <div className="w-full min-h-screen relative bg-[#F2F3F5]">
        <PageRenderer
          ...
          onAlternativeSearchClick={handleAlternativeSearchClick}
          onAlternativeCouponsClick={handleAlternativeCouponsClick}
          onAlternativeLeagueClick={handleAlternativeLeagueClick}
        />
        
        {/* Show Navigation for all standard pages */}
