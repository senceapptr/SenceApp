-- ================================================
-- FIX ADMIN RLS RECURSION (unique version - was 023 duplicate)
-- ================================================
...
CREATE POLICY "Enable insert for authenticated users" ON public.questions FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "Enable update for users based on created_by" ON public.questions FOR UPDATE USING (auth.uid() = created_by);
CREATE POLICY "Enable delete for users based on created_by" ON public.questions FOR DELETE USING (auth.uid() = created_by);
CREATE POLICY "Admins can manage all questions" ON public.questions FOR ALL USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = TRUE)
);

CREATE POLICY "Admins can manage all predictions" ON public.predictions FOR ALL USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = TRUE)
);
CREATE POLICY "Admins can manage all coupons" ON public.coupons FOR ALL USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = TRUE)
);
CREATE POLICY "Admins can manage all leagues" ON public.leagues FOR ALL USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = TRUE)
);
