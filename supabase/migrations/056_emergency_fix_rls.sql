-- ================================================
-- EMERGENCY FIX - REMOVE ALL RLS POLICIES (unique version - was 024 duplicate)
-- ================================================
...
CREATE POLICY "Authenticated users can insert questions" ON public.questions FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
...
CREATE POLICY "Authenticated users can insert predictions" ON public.predictions FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
...
CREATE POLICY "Authenticated users can insert coupons" ON public.coupons FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
...
CREATE POLICY "Authenticated users can insert leagues" ON public.leagues FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "Users can update own leagues" ON public.leagues FOR UPDATE USING (auth.uid() = creator_id);
...
CREATE POLICY "Admin override for profiles" ON public.profiles FOR ALL USING (
  auth.uid() IN (SELECT id FROM public.profiles WHERE id = auth.uid() AND is_admin = TRUE)
);
CREATE POLICY "Admin override for questions" ON public.questions FOR ALL USING (
  auth.uid() IN (SELECT id FROM public.profiles WHERE id = auth.uid() AND is_admin = TRUE)
);
