...
interface PredictionCard3Props {
  question: {
    ...
    totalVotes: number;
    timeLeft: string;
    end_date?: string; // Veritabanından gelen tarih
...
export function PredictionCard3({ question, onVote }: PredictionCard3Props) {
  // Canlı sayaç hook'unu kullan
  const liveTimeLeft = useCountdown(question.end_date);

  // Eğer sonuç girilmişse metni değiştir
  const displayTime = question.result ? 'Sonuçlandı' : (question.end_date ? liveTimeLeft : question.timeLeft);
