...
export function QuestionDetail({ isOpen, onClose, question, onVote }: QuestionDetailProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white overflow-hidden">
      ...
      {/* Scrollable Content */}
      <div className="bg-white rounded-t-3xl -mt-6 relative z-10 flex-1 overflow-auto pb-8 max-h-[calc(100vh-320px)]">
        <div className="p-6">
          {/* Question Title & Info */}
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            {/* Question Author */}
            <div className="flex items-center gap-2 bg-[#F2F3F5] rounded-full px-3 py-2">
              <ImageWithFallback
                src={questionAuthor.avatar}
                alt=""
                className="w-6 h-6 rounded-full"
              />
              <span className="text-[#202020] font-bold text-sm">{questionAuthor.name}</span>
          ...
          <div className="bg-gradient-to-r from-white to-[#F2F3F5] rounded-2xl p-4 mb-6 border border-[#F2F3F5]">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <span className="text-green-600 font-black text-lg">{yesPercentage}%</span>
                <span className="text-[#202020]/70 text-sm">Evet</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[#202020]/70 text-sm">Hayır</span>
                <span className="text-red-600 font-black text-lg">{noPercentage}%</span>
              </div>
            </div>
            ...
            <div className="w-full h-4 bg-gray-200 rounded-full overflow-hidden relative">
              <div 
                className="h-full bg-gradient-to-r from-green-500 to-green-600 rounded-full transition-all duration-1000 absolute left-0"
                style={{ width: `${yesPercentage}%` }}
              />
              <div 
                className="h-full bg-gradient-to-r from-red-500 to-red-600 rounded-full transition-all duration-1000 absolute right-0"
            ...
            <div className="space-y-6">
              <div>
                <div className="flex items-center gap-3 mb-4 p-3 bg-gradient-to-r from-green-50 to-green-100 rounded-xl">
                  ...
                  <h4 className="text-green-700 font-black text-lg">EVET Tarafı</h4>
                </div>
                    ...
                    <div key={index} className="flex items-center justify-between p-4 bg-gradient-to-r from-green-50 to-white rounded-xl border border-green-200 hover:shadow-md transition-all duration-300">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-green-600 text-xs">EVET yatırımcısı</div>
                        </div>
                      </div>
                      <div className="text-green-700 font-black text-lg">{bettor.bet.toLocaleString('tr-TR')} 💎</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center gap-3 mb-4 p-3 bg-gradient-to-r from-red-50 to-red-100 rounded-xl">
                  ...
                  <h4 className="text-red-700 font-black text-lg">HAYIR Tarafı</h4>
                </div>
                <div className="space-y-3">
                  {topBettors.no.map((bettor, index) => (
                    <div key={index} className="flex items-center justify-between p-4 bg-gradient-to-r from-red-50 to-white rounded-xl border border-red-200 hover:shadow-md transition-all duration-300">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="text-[#202020] font-bold">{bettor.user}</span>
                          <div className="text-red-600 text-xs">HAYIR yatırımcısı</div>
                        </div>
                      </div>
                      <div className="text-red-700 font-black text-lg">{bettor.bet.toLocaleString('tr-TR')} 💎</div>
                    </div>
                  ))}
                </div>
              </div>
