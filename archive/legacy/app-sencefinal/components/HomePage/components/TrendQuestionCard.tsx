...
import { getSecondColorFromImage } from '../../utils/imageColorAnalyzer';
...
export function TrendQuestionCard({ question, onQuestionPress, onVote }: TrendQuestionCardProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    getSecondColorFromImage(question.image).then((color) => {
  ...
  return (
    ...
    >
      <View style={[styles.gradientFrameWrapper, { height: 340 + FRAME_WIDTH * 2 }]}>
        <LinearGradient
          colors={[accentColor, accentColor, DARK_GRAY, DARK_GRAY]}
          style={[StyleSheet.absoluteFill, styles.gradientFrameBorder]}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
        />
        <View style={styles.gradientFrameInner}>
          <Animated.View 
            style={[
              styles.card,
...
const styles = StyleSheet.create({
  ...
  gradientFrameWrapper: {
    borderRadius: CARD_RADIUS + FRAME_WIDTH,
    overflow: 'hidden',
    shadowColor: '#000',
